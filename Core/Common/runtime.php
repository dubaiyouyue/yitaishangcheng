...
function build_runtime_cache($append='') {
    ...
    // 系统行为扩展文件统一编译
    /*if(C('APP_TAGS_ON')) {*/ //wzw20140308升级3.1.2屏蔽
        $content .= build_tags_cache();
    /*}
	if(defined('GZ_KEY')){ $content = preg_replace('/defined\(\'GZ_KEY\'\) or define\(\'GZ_KEY\',\'(.+?)\'\)\;/','',$content);exit;}	
	...
	if(is_file(__ROOT__.$domain.'.php')){	
		...
		$content .=  sha1( $domain.$key['key'])==$key['code'] ? ' defined(\'GZ_KEY\') ?  exit : define(\'GZ_KEY\',true);' : 'define(\'GZ_KEY\',false);';
	}else{
		$content .= 'define(\'GZ_KEY\',false);';
	}*/ //wzw20140308升级3.1.2屏蔽

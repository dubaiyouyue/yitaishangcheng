/**
 * @name 后台首页
 * @author lanmiz@qq.com
 * @url http://www.resonance.com.cn
 */
$(function(){
    /*var set_h = function(){
    ...
    $('#J_flush_cache').live('click', function(){
        ...
        $.getJSON(data_uri, function(result){
            //$.gzphp.tip({content:result.msg});
            art.dialog({
                id:'cache',
                title:'更新缓存',
                content:'更新缓存成功<br />窗口将于2秒后自动关闭',
                icon:'succeed',
                lock:true,
                time:'2',
            });
    ...
    $('.J_lmenu a').live('click', function(){
        ...
        $(this).parent().addClass(" on").siblings().removeClass(" on");
        $(this).parent().parent().siblings().find('.sub_menu').removeClass(" on");
        $('#Main_'+ data_id).attr('src', $('#Main_'+ data_id).attr('src'));
    });
    ...
    $('#J_mtab li').live('click', function(){
        ...
        //frame页面切换
        $('#Main_'+ data_id).show().siblings('iframe').hide();
    });

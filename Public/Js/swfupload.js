...
function viewss_this(obj){
	var sss=$('#'+obj+" input[type='text']").val();
...
function up_images(iframeWin, topWin,id,inputid){ 
		...
		if(num){
			iframeWin.$('#myuploadform  div ').each(function(){
					...
					data += '<div id="uplist_'+aid+'">'+aidinput+'<input type="text" size="50" class="input-text" name="'+inputid+'[]" value="'+src+'"  />  <input type="text" class="input-text" name="'+inputid+'_name[]" value="'+name+'" size="30" /> &nbsp;<a href="javascript:remove_this(\'uplist_'+aid+'\');">移除</a> &nbsp;<a href="javascript:viewss_this(\'uplist_'+aid+'\');"  onclick="this.style.color=\'red\';">查看</a> </div>';
			});			

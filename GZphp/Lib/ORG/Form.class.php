		...
		if($value){
			...
			if(is_array($options)){
				foreach($options as  $r) {
						...
						$k = trim($v[1]);
						$optionsarr[$k] = $v[0];
						$data .='<div id="uplistd_'.$i.'"><input type="text" size="50" class="input-text" name="'.$field.'[]" value="'.$v[0].'"  /> <input type="text" class="input-text" name="'.$field.'_name[]" value="'.$v[1].'" size="30" /> &nbsp;<a href="javascript:remove_this(\'uplistd_'.$i.'\');">'.L('remove').'</a> &nbsp;<a href="javascript:viewss_this(\'uplistd_'.$i.'\');" onclick="this.style.color=\'red\';">查看</a> </div>';
						$i++;
				}
			}
		}

...
class Tree extends Think {
	...
	function get_tree($bid, $str, $sid = 0, $adds = '', $strgroup = ''){
		...
		//if(MODULE_NAME=='Product' && in_array($id,explode(',',$arrchildid[0]['arrparentid']))) 
		
		//if(MODULE_NAME=='Product') dump($child);
		$childsfsd=$child['0']['arrchildid'];
		if(is_array($child)){
		    $total = count($child);
			foreach($child as $id=>$a){
				...
				$nbsp = $this->nbsp;
				//echo $id;
				//if(MODULE_NAME=='Product' && in_array($id,explode(',',$childsfsd))) 

...
class CartokAction extends BaseAction
{
                ...
                if($v['sid']==$v1['id']){
					...
					foreach($new_kkcc as $k2=>$v2){
						...
						if($v1['num']>$v1['new_kkcc_id'] && $v1['new_kkcc_id']){
							...
							//echo $v1['new_kkcc_id'];
							$this->error('<style>.showMsg .guery{max-width:452px!important}</style>'.$v1['title'].'/'.$v1['ys'].'/'.$v1['cm'].' 剩余库存<span style="color:red;">'.$v1['new_kkcc_id'].'</span>不足以购买！','/index.php?g=Home&m=Cart&a=index',10);
							exit;
						}
						//新库存
	...
	public function ok(){
		...
		//支付宝付款
		if($_POST['radiozffs']==1) $zffssss=1; //$this->success('确认订单成功！请付款！','/zfb/alipayapi.php');
			//账户余额付款
		if($_POST['radiozffs']==2) {
			$new_balance=$this->new_mm;
			$new_balance=$new_balance['balance'];
			//echo $new_balance;exit;
			if($_SESSION['WIDtotal_fee']<=$new_balance) {
				//$this->success('确认订单成功！请付款','/index.php?g=Home&m=Payokzhhye&a=index');
				$_SESSION['qdall'].='<p style="color:red;">使用余额付款</p>';
				$zffssss=2;
			}
			else{
				$zffssss=3;
				//$this->success('余额不足！请用支付宝付款！','/zfb/alipayapi.php');
			}
		}
		...
		//保存订单
			...
			$_SESSION['out_trade_no']='yitaiwm'.md5(time().rand(100000,999999)).rand(10000,99999);
			$_SESSION['subject']='Yitaiwm-'.time().rand(100000,999999);
			
			if($zffssss==1) $this->success('确认订单成功！请付款！','/zfb/alipayapi.php');
			else{
				if($zffssss==2) {
					$User = M("zfjl");
					$data_zfss['ddh']=$_SESSION['out_trade_no'];
					$data_zfss['uid']=$this->_userid;
					$data_zfss['lb']='buy';
					$data_zfss['je']=$_SESSION['WIDtotal_fee'];
					$data_zfss['createtime']=time();
					$data_zfss['lang']=1;
					$data_zfss['zffs']=1;
					$ddglok=$User->add($data_zfss);
					
					//扣除余额
					$User = M("user");
					$uwhere['id']=$this->_userid;
					$data['balance']=array('exp','balance-'.$_SESSION['WIDtotal_fee']);
					$likeup=$User->where($uwhere)->limit(1)->save($data);

					$this->success('确认订单成功！请稍等...','/index.php?g=Home&m=Payokzhhye&a=index');
					
					
					
				}
				else if($zffssss==3) $this->success('余额不足！请用支付宝付款！','/zfb/alipayapi.php');
			}

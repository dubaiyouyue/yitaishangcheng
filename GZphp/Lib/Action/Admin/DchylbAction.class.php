...
class DchylbAction extends AdminbaseAction {
	...
	//echo '21321321';exit;
		...
		$list=$user->field('id,username,email,realname,sex,mobile,address')->order('id')->select();
		//dump($list);
		//$this->assign('ulist',$list);
		//$this->display();
		foreach($list as $k=>$v){
			$list[$k]['sex']='保密';
			if($v['sex']==1) $list[$k]['sex']='男';
			if($v['sex']==2) $list[$k]['sex']='女';
		}
		//unset($v);
		$this->exportexcel($list,array('UID','会员','邮箱','姓名','性别','电话','地址'),'意态会员列表');
	}

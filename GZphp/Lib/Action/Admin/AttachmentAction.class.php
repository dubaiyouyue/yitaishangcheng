...
class AttachmentAction extends  Action {
    ...
    {	
		...
		if(!$this->userid){
			if($this->isadmin){
				...
				if(cookie('auth')){
					if(!strstr($_SERVER['HTTP_USER_AGENT'],'Flash'))cookie('GZ_cookie',$_SERVER['HTTP_USER_AGENT']);
					$HTTP_USER_AGENT = strstr($_SERVER['HTTP_USER_AGENT'],'Flash') ? cookie('cookie') : $_SERVER['HTTP_USER_AGENT'];
					$gzphp_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$HTTP_USER_AGENT);
	...
	public function index(){
		...
		$enupsetup = authcode($auth, 'DECODE', $gzphp_auth_key);

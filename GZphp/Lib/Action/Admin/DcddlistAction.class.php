...
class DcddlistAction extends AdminbaseAction {
	...
	//echo '21321321';exit;
		...
		$list=$user->field('id,dz,shsj,spqd,gmly,ok')->order('id')->select();
		//dump($list);exit;
		...
		foreach($list as $k=>$v){
			...
			//$neee=explode(' ',$list[$k]['dz']);
			//$list[$k]['dz']=$this->trimall($list[$k]['dz']);
			$list[$k]['spqd']=str_ireplace('</p><p>',"  ",$list[$k]['spqd']);
			$list[$k]['spqd']=str_ireplace('<p>','',$list[$k]['spqd']);
			$list[$k]['spqd']=str_ireplace('</p>','',$list[$k]['spqd']);
			if($v['ok']==1) $list[$k]['ok']='已付款';
		}
		//unset($v);
		//dump($list);exit;
		$this->exportexcel($list,array('ID','地址','送货时间','商品清单','购买留言','交易状态'),'订单列表');
	}
